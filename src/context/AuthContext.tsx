...
function mapOfficerUser(authUser: AuthUserResponse): OfficerUser {
  ...
  const departmentName = profile?.department?.name || authUser.department || 'Municipality / Sanitation';
  const departmentId = profile?.department_id || authUser.department_id;
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
      ...
      if (token && !USE_MOCK_DATA) {
        ...
        if (result.success && result.data) {
          ...
          if (authUser.role === 'citizen') {
            ...
            setOfficerUser(mapOfficerUser(authUser));
  ...
  const loginCitizen = async (credentials: LoginCredentials) => {
    ...
    if (!USE_MOCK_DATA) {
      ...
      if (res.success && res.data) {
        ...
        if (authUser.role && authUser.role !== 'citizen') {
          setIsLoading(false);
          return { success: false, error: `Access denied. This account has role "${authUser.role.toUpperCase()}", not "CITIZEN".` };
        }
        ...
        setUser(citizenData);
        if (credentials.rememberMe) localStorage.setItem(CITIZEN_STORAGE_KEY, JSON.stringify(citizenData));
        setIsLoading(false);
        return { success: true };
      }
      setIsLoading(false);
    ...
    setUser(mockUser);
    if (credentials.rememberMe) localStorage.setItem(CITIZEN_STORAGE_KEY, JSON.stringify(mockUser));
    setIsLoading(false);
    return { success: true };
  };
  ...
  const requestPasswordReset = async (email: string) => {
    await new Promise((res) => setTimeout(res, 400));
    if (email.includes('notfound')) return { success: false, error: 'No citizen account found with this email address.' };
    return { success: true };
  };
  ...
  const loginOfficer = async (credentials: LoginCredentials) => {
    ...
    if (!USE_MOCK_DATA) {
      ...
      if (res.success && res.data) {
        ...
        if (authUser.role && authUser.role !== 'officer') {
          setIsLoading(false);
          return { success: false, error: `Access denied. This account has role "${authUser.role.toUpperCase()}", not "OFFICER".` };
        }
        const officerData = mapOfficerUser(authUser);
        setOfficerUser(officerData);
        if (credentials.rememberMe) localStorage.setItem(OFFICER_STORAGE_KEY, JSON.stringify(officerData));
        setIsLoading(false);
        return { success: true };
      }
      setIsLoading(false);
    ...
    if (credentials.rememberMe) localStorage.setItem(OFFICER_STORAGE_KEY, JSON.stringify(mockOfficer));
    setIsLoading(false);
    return { success: true };
  };
  ...
  const loginAdmin = async (credentials: LoginCredentials) => {
    ...
    if (!USE_MOCK_DATA) {
      ...
      if (res.success && res.data) {
        ...
        if (authUser.role && authUser.role !== 'admin') {
          setIsLoading(false);
          return { success: false, error: `Access denied. This account has role "${authUser.role.toUpperCase()}", not "ADMIN".` };
        }
        ...
        if (credentials.rememberMe) localStorage.setItem(ADMIN_STORAGE_KEY, JSON.stringify(adminData));
        setIsLoading(false);
        return { success: true };
      }
      setIsLoading(false);
    ...
    if (credentials.rememberMe) localStorage.setItem(ADMIN_STORAGE_KEY, JSON.stringify(mockAdmin));
    setIsLoading(false);
    return { success: true };
  };
