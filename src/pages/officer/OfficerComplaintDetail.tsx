...
export function OfficerComplaintDetail() {
    ...
    if (res.success && res.data) {
      ...
      setApiError(
        res.error?.includes('403')
          ? 'You are not authorized to view or modify this complaint.'
    ...
    if (!USE_MOCK_DATA) {
      ...
      if (!res.success) {
        setToastMessage({
          ...
          desc: res.error || 'Unable to update complaint status on server.',
        });
        return;
      }
      fetchComplaintData();
  ...
  const handleAddProgressNote = async (e: React.FormEvent) => {
    ...
    if (!USE_MOCK_DATA) {
      ...
      if (!res.success) {
        setProgressNoteError(res.error || 'Failed to save progress note to server.');
        return;
      }
      fetchComplaintData();
    } else {
  ...
  const handleFinalConfirmResolve = async () => {
    ...
    if (!USE_MOCK_DATA) {
      ...
      if (!res.success) {
        setResolutionNoteError(res.error || 'Unable to submit resolution report to server.');
    ...
    setToastMessage({
      ...
      desc: `Case ${complaint.id} has been marked RESOLVED with completion proof attached.`,
    });
  };

  if (!USE_MOCK_DATA && isLoading) {
    return <LoadingState title="Loading complaint details..." description="Retrieving case data and activity logs from server." />;
  }
    ...
    return (
      <div className="max-w-3xl mx-auto py-12 space-y-4">
        <ErrorMessage
          ...
          message={apiError || 'Failed to connect to the municipal record server. Please retry.'}
        />
        <div className="text-center">
          <Button variant="outline" size="sm" onClick={fetchComplaintData} leftIcon={<RotateCcw className="w-3.5 h-3.5" />}>
            Retry Loading
          </Button>
        </div>
      </div>
    ...
    return (
      <div className="max-w-3xl mx-auto py-12 text-center space-y-4 bg-white rounded-lg border border-slate-200 p-8">
        ...
        <p className="text-xs text-slate-500 max-w-sm mx-auto">
          The requested complaint reference "{id}" does not exist in your officer queue.
        </p>
  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto pb-16">
      ...
      {toastMessage && (
        <div className="fixed bottom-6 right-6 z-50 max-w-md bg-slate-900 text-white p-4 rounded-xl shadow-xl border border-slate-700 flex items-start gap-3 animate-in slide-in-from-bottom duration-300">
      ...
      {/* Page Header */}
      <PageHeader
        title={`Manage Complaint ${complaint.id}`}
        description="Review reported details, update status, record field notes, and log resolution proof."
        breadcrumbs={[
      ...
      <Card className="shadow-2xs text-left">
        ...
        <CardContent className="space-y-6 pt-6 text-left">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 p-4 bg-slate-50 rounded-lg border border-slate-200 text-xs">
            ...
            <div>
              ...
              <span className="font-bold text-slate-900 flex items-center gap-1.5">
                ...
                {complaint.citizenName} ({complaint.ward})
              </span>
            </div>
          </div>
          ...
          {/* Action Toolbar: Update Status & Mark Resolved */}
          <div className="p-4 rounded-xl bg-slate-900 text-white space-y-3">
            <div className="flex items-center justify-between flex-wrap gap-2">
              <span className="text-xs font-bold uppercase tracking-wider text-slate-300">
                Officer Actions & Workflow
              </span>
              <span className="text-[11px] text-slate-400">Current Status: {complaint.status}</span>
            </div>
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
              ...
              <div className="flex-1">
                <select
                  disabled={isUpdatingStatus || complaint.status === 'RESOLVED'}
                  value={selectedStatus || complaint.status}
                  onChange={handleStatusSelect}
                  className="w-full text-xs font-semibold bg-slate-800 text-white border border-slate-700 rounded-lg px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-blue-500 cursor-pointer disabled:opacity-50"
                >
                  <option value="NEW">Status: NEW (Unassigned/Triage)</option>
                  <option value="ASSIGNED">Status: ASSIGNED (In Triage)</option>
                  <option value="IN_PROGRESS">Status: IN_PROGRESS (Field Technician On-Site)</option>
                  <option value="RESOLVED">Status: RESOLVED (Mark Complete & Close)</option>
                </select>
              </div>
      ...
      {complaint.status === 'RESOLVED' && complaint.resolution && (
        <Card className="border-2 border-emerald-300 shadow-md bg-emerald-50/40 text-left">
          ...
          <CardContent className="p-5 space-y-4 text-xs">
            ...
            <div className="flex items-center gap-2 text-slate-700 pt-1 font-medium">
              <ShieldCheck className="w-4 h-4 text-emerald-600" />
              <span>Resolved by Officer <strong>{complaint.resolution.officerName}</strong></span>
            </div>
          </CardContent>
        </Card>
      )}
        ...
        <Card className="shadow-2xs text-left">
          ...
          <CardContent className="p-5 space-y-4">
            <form onSubmit={handleAddProgressNote} className="space-y-3">
              <Textarea
                rows={3}
                placeholder="Log field update (e.g. Dispatched crew to inspect pavement slab, estimated completion by 4 PM)..."
              ...
              <div className="flex items-center justify-between">
                <span className="text-xs text-slate-500">
                  Notes are logged publicly to the citizen tracking timeline.
                </span>
                <Button
                  type="submit"
                  variant="primary"
      ...
      <Card className="shadow-2xs text-left">
        <CardHeader className="border-b border-slate-200 bg-slate-50">
          <CardTitle className="text-base font-bold text-slate-900 flex items-center gap-2">
            ...
            Complaint Activity & Timeline History
          </CardTitle>
        </CardHeader>
          ...
          <div className="relative pl-6 sm:pl-8 space-y-6 before:absolute before:left-2.5 sm:before:left-3.5 before:top-2 before:bottom-2 before:w-0.5 before:bg-slate-200">
            {complaint.timeline.map((entry) => (
              <div key={entry.id} className="relative group">
                <div className="absolute -left-6 sm:-left-8 top-0.5 w-6 h-6 sm:w-7 sm:h-7 rounded-full bg-slate-900 text-white flex items-center justify-center border-2 border-white shadow-2xs">
      ...
      {resolveModalOpen && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-xl max-w-lg w-full p-6 space-y-5 shadow-2xl animate-in fade-in text-left">
            ...
            <form onSubmit={handleStartResolve} className="space-y-4 text-xs">
              <div className="space-y-1">
                <label className="font-semibold text-slate-700 block">
                  Work Completed Resolution Note <span className="text-rose-600">*</span>
                </label>
                <Textarea
                  rows={3}
                  placeholder="Describe the action taken (e.g. Pavement slab re-laid and sealed, area cleared)..."
              ...
              <div className="space-y-2">
                <label className="font-semibold text-slate-700 block">
                  Attach Field Resolution Photo Proof (Optional)
                </label>
                <input
                  type="file"
                  ref={fileInputRef}
                ...
                {resolutionPhotoPreview ? (
                  ...
                  <div
                    ...
                    className="border-2 border-dashed border-slate-300 hover:border-blue-500 rounded-lg p-6 text-center bg-slate-50 cursor-pointer hover:bg-blue-50/40 transition-colors"
                  >
                    <Upload className="w-6 h-6 text-slate-400 mx-auto mb-1" />
                    <p className="font-bold text-slate-700">Click to upload completion photo</p>
      ...
      {confirmModalOpen && (
        <div className="fixed inset-0 z-50 bg-slate-900/70 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-xl max-w-md w-full p-6 space-y-4 shadow-2xl text-left">
            <div className="flex items-center gap-3 text-amber-600">
              <AlertTriangle className="w-8 h-8 shrink-0" />
              <div>
                <h3 className="font-bold text-base text-slate-900">Confirm Case Resolution</h3>
                <p className="text-xs text-slate-500">
                  Marking this case as RESOLVED will close the complaint and notify the citizen.
                </p>
              </div>
            </div>

            <div className="p-3 bg-slate-50 rounded-lg border border-slate-200 text-xs space-y-1">
              <span className="font-semibold text-slate-700 block">Resolution Note Preview:</span>
              <p className="text-slate-600 italic">"{resolutionNote}"</p>
            </div>
          ...
          <div className="bg-white rounded-xl max-w-md w-full p-6 space-y-4 shadow-2xl text-left">
            ...
            <div className="space-y-3 text-xs">
              <div className="p-3 rounded-lg bg-slate-50 border border-slate-200 space-y-1">
                <span className="font-semibold text-slate-500 block uppercase text-[10px]">Address</span>
                <p className="font-bold text-slate-900">{complaint.location}</p>
              </div>

              {complaint.coordinates && (
                <div className="p-3 rounded-lg bg-blue-50 border border-blue-200 space-y-2">
                  <span className="font-semibold text-blue-900 block uppercase text-[10px]">Coordinates</span>
                  <div className="font-mono text-sm text-slate-900 flex items-center justify-between">
                    <span>{complaint.coordinates.lat.toFixed(6)}°, {complaint.coordinates.lng.toFixed(6)}°</span>
                    <button
                      type="button"
                      onClick={() => {
                        if (complaint.coordinates) {
                          navigator.clipboard.writeText(`${complaint.coordinates.lat},${complaint.coordinates.lng}`);
                          setCopiedCoords(true);
                          setTimeout(() => setCopiedCoords(false), 2000);
                        }
                      }}
                      className="text-blue-700 hover:underline flex items-center gap-1 font-sans text-xs"
                    >
            ...
            <div className="flex justify-end pt-2">
              <Button size="sm" variant="outline" onClick={() => setLocationModalOpen(false)}>
                Close
              </Button>
            </div>
          </div>
