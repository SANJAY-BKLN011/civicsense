  ...
  const complaint = !USE_MOCK_DATA
    ? apiComplaint
      ? {
          ...
          category: typeof apiComplaint.category === 'string' ? apiComplaint.category : (apiComplaint.category as any)?.name || 'Municipal Triage',
          department: typeof apiComplaint.department === 'string' ? apiComplaint.department : (apiComplaint.department as any)?.name || officerUser?.department || 'Municipality / Sanitation',
          submittedDate: String(apiComplaint.submittedDate || (apiComplaint.createdAt ? new Date(apiComplaint.createdAt).toLocaleDateString() : 'Aug 20, 2026')),
          submittedTime: String(apiComplaint.submittedTime || '10:30 AM'),
          citizenName: typeof apiComplaint.citizenName === 'string' ? apiComplaint.citizenName : (apiComplaint as any).citizen?.name || 'Sanjay Patel',
          ...
          thumbnailIcon: typeof apiComplaint.thumbnailIcon === 'string' ? apiComplaint.thumbnailIcon : '📌',
          ...
          ward: typeof apiComplaint.ward === 'string' ? apiComplaint.ward : (apiComplaint.ward as any)?.name || 'Ward 12 - Central District',
          ...
          timeline: (apiComplaint.timeline || [
            { id: `tl-1`, timestamp: String(apiComplaint.submittedDate || 'Aug 20, 10:30 AM'), title: 'Complaint Submitted', description: 'Issue reported by citizen and logged into central triage system.', author: typeof apiComplaint.citizenName === 'string' ? apiComplaint.citizenName : 'Citizen', type: 'submission' },
          ]).map((t, idx) => ({
